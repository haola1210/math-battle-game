...
import { Circle as MafsCircle, type vec } from 'mafs';
...
import { didCollideCircle } from './utils';
...
export default function Circle({ color, position, r }: IObstacle) {
  //
    ...
    const bulletSub = event$.subscribe({
      next({ action, payload }) {
        if (action === BULLET_ACTION.FLYING) {
          ...
          if (!isInsideAnyHole && didCollideCircle(position, r, pVec)) {
